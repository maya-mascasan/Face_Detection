...
void FindFaceAndDrawBox(Mat src, Mat skin) {
	...
	if (goodFace != -1) {
		...
		int final_min_r = face.min_r - (int)(boxHeight * 0.3f);
		int final_max_r = face.max_r + (int)(boxHeight * 0.5f);
		int final_min_c = face.min_c - (int)(boxWidth * 0.2f);
		int final_max_c = face.max_c + (int)(boxWidth * 0.2f);
...
void detectSkin(bool ok) {
	...
	while (openFileDlg(fname))
	{
		...
		int height = src.rows;
		int width = src.cols;
		Mat skinMask = Mat::zeros(height, width, CV_8UC1);
